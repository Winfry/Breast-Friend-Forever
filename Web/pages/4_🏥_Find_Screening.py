...
# Process hospital data with enhanced information
if hospital_df is not None:
    clinics_data = []
    
    # Limit to first 200 facilities for better performance
    limited_df = hospital_df.head(200)
...
if filtered_clinics:
    ...
    # Add pagination
    items_per_page = 10
    total_pages = (len(filtered_clinics) + items_per_page - 1) // items_per_page
    ...
    if total_pages > 1:
        ...
        st.write(f"Showing facilities {start_idx + 1}-{end_idx} of {len(filtered_clinics)}")
...
# ... rest of your existing code for appointments, tips, quick actions, etc.
