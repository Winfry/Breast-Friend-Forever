...
# CORRECTED message display function
def display_message_card(message):
    """Display a message card without HTML escaping issues"""
    ...
    # Use st.markdown to create the card structure
    st.markdown(f"""
        <div class="{card_class}">
            <div style="display: flex; align-items: center; margin-bottom: 1rem;">
                <div class="user-avatar">{message['avatar']}</div>
                <div>
                    <h4 style="margin: 0 0 0.3rem 0; color: #1F2937;">{message['user']}</h4>
                    <p style="margin: 0; color: #6B7280; font-size: 0.9rem;">{time_display}</p>
                </div>
            </div>
            <div class="message-content">{message['message']}</div>
            <div style="display: flex; gap: 2rem; margin-bottom: 1rem;">
                <div class="reaction-count" style="color: #EF4444;">💖 {message['reactions']['hearts']}</div>
                <div class="reaction-count" style="color: #F59E0B;">🌟 {message['reactions']['sparks']}</div>
                <div class="reaction-count" style="color: #8B5CF6;">🌈 {message['reactions']['rainbows']}</div>
            </div>
        </div>
    """, unsafe_allow_html=True)
...
# Display each message using the corrected function
